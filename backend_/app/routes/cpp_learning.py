...
def add_learning_content():
    try:
        ...
        db.session.rollback()
        logger.error(f"Error adding learning content: {str(e)}")
        return jsonify({"error": "Failed to add learning content"}), 500

# Route to fetch quiz questions for a given concept
@cpp_learning.route('/get_quiz/<concept>', methods=['GET'])
